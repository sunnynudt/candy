...
| 命令           | 语法                                                                   | 说明                                                                                                                                                                                                                                                                                         |
| -------------- | ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/model`       | `/model <deepseek-flash \| deepseek-pro \| minimax-m3 \| 配置模型 id>` | 选择主模型；键入裸 `/model` 会先显示“查看当前模型”和模型候选（含 `models.json` 配置的用户模型），直接 Enter 仍只列出当前模型与可选模型；候选列表中当前使用的模型会以 ✓ 标注，“查看当前模型”行也会显示当前模型名。 |
| `/attach`      | `/attach <absolute-path>`                                              | 为下一个任务附加图片（仅声明支持图片输入的模型）；也可复制图片后按 `Ctrl+V`                                                                                                                                                                                                               |
...
| 命令                 | 语法                                             | 说明                                                                                                                                    |
| -------------------- | ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| `/steer`             | `/steer <text>`                                  | 显式插入当前活动 turn（普通输入默认排队到下一轮）                                                                                       |
| `/follow-up`         | `/follow-up <text>`                              | 排队一条追问（任务运行中的普通输入默认也按此排队）                                                                                      |
...
| 命令         | 语法                                    | 说明                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------ | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/access`    | `/access [review\|safe\|current\|full]` | 选择 Candy 如何工作：`review` 只读分析；`safe` 默认在安全副本中工作且网络逐条确认；`current` 直接在当前工作区工作且网络开发命令逐条确认；`full`（通过验证的 macOS 或 Windows Full Access Backend）经两步确认后放宽命令沙箱（文件+网络），不再逐条确认普通命令；它与 `current` 可叠加形成 Codex 式宽沙箱直连开发流，`/access safe` 是关闭 Full access 的出口。两个可写模式均可离线运行已有本地检查 |
...
- 输入区根据状态显示 `开始任务`、`Candy 正在处理`、`需要你的确认` 或 `继续任务`，底部只保留当前场景需要的命令提示。键入 `/` 打开 Candy 自有命令列表。任务运行中（`Candy 正在处理`）直接输入普通文本并回车会默认排队到下一轮（排队内容显示在状态提示行下方的“排队”区，不混入执行过程文本；轮到下一轮或任务结束/暂停后清空），不会打断当前轮；需要插话到当前轮请用 `/steer <text>`。
...
- `Ctrl+V` 只在用户主动按键时读取系统剪贴板中的图片，并把它直接存为 Candy 自有附件（不读取剪贴板文本、不写临时外部图片文件）；粘贴成功后显示 attachment id。图片附件需要选择声明支持图片输入的模型，否则创建任务前会给出提示。
...
- 已批准主机上的本地 `npm run` 等命令在两个可写访问模式下离线可用，复用已有依赖而不自动下载。两个可写工作区都可对单条网络开发命令确认后执行依赖安装或源代码拉取；发布和含凭据命令仍被拒绝。Full Access 在通过验证的 macOS 或 Windows Backend 上一次确认后允许文件、命令与网络连续执行。依赖不可用时，任务会明确提示先在源工作区安装依赖后新建任务，Candy 不会自行下载；如果平台能力未通过 G2，Candy 会显示具体原因。
