...
- 输入区根据状态显示 `开始任务`、`Candy 正在处理`、`需要你的确认` 或 `继续任务`，底部只保留当前场景需要的命令提示。键入 `/` 打开 Candy 自有命令列表。任务运行中（`Candy 正在处理`）直接输入普通文本并回车会默认排队到下一轮（显示 `[follow-up] ... queued`），不会打断当前轮；需要插话到当前轮请用 `/steer <text>`。
