...
- 用户消息与模型回复分别以 `你`、`Candy` 标识。模型回复（assistant 流式输出）以 markdown 渲染（标题加粗、引用/链接/分割线暗色、代码块保留原文）；宽终端上的普通回复限制为 112 列阅读宽度，状态行、diff 等证据仍可使用完整终端宽度。
- 同一次工具调用在实时 transcript 中只占一条活动记录：开始、更新和完成会就地更新；完成后保留有界、脱敏的最终工具证据。等待审批使用独立的警示层级和可复制命令，不与普通工具活动混排。
