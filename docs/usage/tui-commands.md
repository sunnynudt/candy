...
| 命令         | 语法                                    | 说明                                                                                                                                                                                                                                                                                                                   |
| ------------ | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/access`    | `/access [review\|safe\|current\|full]` | 选择 Candy 如何工作：`review` 只读分析；`safe` 默认在安全副本中工作且网络逐条确认；`current` 直接在当前工作区工作且默认不开放网络命令；`full`（macOS 预览）放宽命令沙箱（文件+网络），与 `current` 可叠加形成 Codex 式宽沙箱直连开发流，`/access safe` 是关闭 Full access 的出口。两个可写模式均可离线运行已有本地检查 |
...
- 已批准 macOS 主机上的本地 `npm run` 等命令在两个可写访问模式下离线可用，复用已有依赖而不自动下载。安全工作区的网络操作仍逐条确认；当前工作区默认不开放网络命令（Full access 生效时除外）。依赖不可用时，任务会明确提示先在源工作区安装依赖后新建任务，Candy 不会自行下载；如果平台能力未通过 G2，Candy 会显示具体原因。
