...
| ACC-06 | Pass (deterministic subset) | Three active slots, FIFO promotion/reorder, four-task Windows concurrency smoke, and worktree separation fixtures pass. Full provider-rate-limit and dual-platform concurrency evidence remains open. |
...
| ACC-08 | Partial/Blocked | Git worktree create/apply/discard/restart fixtures, binary/untracked Apply, dirty/changed-base/conflict guards, Windows path/reparse fixtures, and packaged coding-journey Apply (uncommitted, index untouched) pass. Full conflict/binary/recovery matrix and both-platform review remain open. |
| ACC-09 | Partial/Blocked | Packaged Windows Browser allowlist, revision fencing, typed actions, screenshot attachment id, default-deny navigation/popup/permission/download, prompt-injection inertness, navigation race, Take Control conflict rejection, and page-marker isolation pass. Reliable physical input-origin detection remains Blocked; explicit Take Control fallback remains the accepted transfer. |
...
| ACC-11 | Partial/Blocked | SQLite WAL/schema v10 migration, transcript persistence across restart, owner interruption/revision fencing, attachment recovery, packaged Node/app-server recovery, and app-server child restart race fix pass. Upgrade/rollback, backup/restore, disk-full, corrupted-data, and task/attachment deletion policy fixtures remain open/Blocked where product scope requires them. |
