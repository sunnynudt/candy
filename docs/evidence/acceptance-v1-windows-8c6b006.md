...
| ACC-05 | Partial/Blocked | Revision fencing, owner protection, packaged sequential cross-client handoff, attachment restart recovery, reopened transcript persistence, and the TUI/Desktop shared app-data path (Desktop now resolves userData through `resolveDefaultAppDataRoot()`, same as TUI/app-server) pass. Packaged TUI/Desktop same-session journey across both platforms and full cross-platform restart matrix remain open. |
