...
| Full access as an earned package | Broad access is a capability claim, not a rename of a current-workspace option. | Do not expose Full access until it can truthfully provide the advertised filesystem and network scope. Provider credential isolation and Candy-managed publication controls remain non-negotiable in every mode. | **Platform-gated; unavailable until proven**. | Separate macOS and Windows native negative matrices: path escape, symlink/reparse race, network grant/deny, inherited children, cancellation, timeout, parent loss, redaction, and credential isolation. | Both platform capability attestations and the product warning/recovery journey are accepted. |
