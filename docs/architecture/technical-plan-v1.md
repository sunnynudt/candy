...
| Electron | `43.2.0`, embedding Node `24.18.0` | selected Gate baseline for current macOS Tahoe `26.x` Apple Silicon and Windows 11 |
...
app-server composition root ----> platform/native-process ----> versioned JSONL stdio ----> native sandbox-runner
                                      ^
                                      |
                              runtime CommandRunner port
```

Composition roots create concrete Pi, Browser, credential, and native-process adapters. `runtime` does not import Electron, Pi, a provider SDK, or a platform-specific runner. The platform-native process adapter owns the JSONL client, runner path resolution, environment rejection, bounded transport, unconditional raw active-secret request scanning, scanning of subsequent JSON representations only while each candidate fits the v1 request-line limit, and OS cancellation strategy; Runtime owns only the `CommandRunner` port and common `CommandValidator` policy. `protocol` contains no I/O or business policy.
...
WP2 adds the exact direct dependency `@earendil-works/pi-tui@0.84.1` to `apps/tui` and enforces a source allowlist: `packages/pi-adapter` may import only `@earendil-works/pi-coding-agent`, `apps/tui` may import only `@earendil-works/pi-tui`, and other source packages may not import Pi. `CandyTuiSurface` wraps only the public `TuiAltScreen`, `ProcessTerminal`, `Editor`, `Text`, and key-matching APIs. `InteractiveTui` continues to own commands, tasks, scheduling, Runtime invocation, and session ownership; no Pi interactive mode, client/protocol/server, plugin, or second session store is introduced. The surface rejects the three Pi debug/log environment variables, receives an explicit Candy app-data log directory, redacts transcript output, and exposes an injectable FakeTerminal for lifecycle tests. Normal, Ctrl+C, startup-error, and runtime-error restoration are deterministic evidence; raw mode, paste, Chinese input, resize, IME, and terminal recovery remain macOS/Windows real-terminal acceptance work.
...
`@candy/platform` supplies Candy-owned application-data paths, OS credential-store access, SQLite task persistence and leases, child-environment hygiene, and the `native-process` adapter. The adapter owns the platform-specific JSONL client, runner executable resolution, process startup, cancellation, and native-platform selection. Runtime consumes it only through the platform-neutral `CommandRunner` port.

It is not a facade for every host concern: Electron main retains narrow packaging and capture branches. Those are host/UI mechanics; native command startup and `process.platform` selection are centralized in `@candy/platform`.
...
| Native command containment and process-tree lifetime | `platform/native-process` implements one versioned JSONL client and one Rust crate; Runtime supplies `CommandRunner` and `CommandValidator` | macOS uses a default-deny Seatbelt profile and process-group cancellation; Windows uses `CreateProcessW` plus a Job Object and reparse-point checks | TypeScript client, runner path, and platform cancellation are centralized in `@candy/platform`; JSONL fields/version/error semantics and Rust backends are unchanged. G2 remains incomplete: Windows has no proven OS-level no-network containment, and both platforms still need the remaining packaging/security evidence. |
...
Runtime provider code obtains a short-lived secret lease from the Platform credential adapter or approved temporary process environment. The secret is used only to build authentication for the allowlisted provider host and is then released. Tool subprocess environments are constructed from an allowlist rather than copied from `process.env`; all active provider variables and known secret values are removed before spawn. The native-process adapter rejects an active secret before spawn when it appears raw, regardless of raw byte length, or through any repeatedly JSON-stringified representation that fits the native v1 request-line limit, across executable, arguments, paths, environment entries, and the serialized request.
...
- platform: current macOS Tahoe `26.x` Apple Silicon and Windows 11 credential, lease, process-tree cancellation, path, and app-data tests;
...
- end to end: the same saved task fixture loads on the current macOS Tahoe `26.x` Apple Silicon host and Windows 11 after workspace remapping.
