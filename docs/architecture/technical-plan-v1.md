...
| Electron | `43.2.0`, embedding Node `24.18.0` | selected Gate baseline for macOS Sequoia 15+ and Windows 11 |
...
- platform: macOS Sequoia 15+ and Windows 11 credential, lease, process-tree cancellation, path, and app-data tests;
...
- end to end: the same saved task fixture loads on macOS Sequoia 15+ and Windows 11 after workspace remapping.
...
- **G1 Persistence — conditional**: Node 22 `node:sqlite` is selected, but TUI/app-server import, concurrent fencing, WAL recovery, migration, protocol-only Electron access, and packaged macOS Sequoia 15+/Windows 11 tests remain open.
...
- Candy V1 requires macOS Sequoia 15 or newer; Electron 43 remains the Gate baseline without compatibility work for macOS 13 or 14.
