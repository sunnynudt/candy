...
Composition roots create concrete Pi, Browser, credential, and native-process adapters. `runtime` does not import Electron, Pi, a provider SDK, or a platform-specific runner. The platform-native process adapter owns the JSONL client, runner path resolution, environment rejection, bounded transport, active-secret request scanning across all JSON representations that fit the v1 request-line limit, and OS cancellation strategy; Runtime owns only the `CommandRunner` port and common `CommandValidator` policy. `protocol` contains no I/O or business policy.
...
Runtime provider code obtains a short-lived secret lease from the Platform credential adapter or approved temporary process environment. The secret is used only to build authentication for the allowlisted provider host and is then released. Tool subprocess environments are constructed from an allowlist rather than copied from `process.env`; all active provider variables and known secret values are removed before spawn. The native-process adapter also rejects an active secret before spawn when it appears raw or through any repeatedly JSON-stringified representation that fits the native v1 request-line limit, across executable, arguments, paths, environment entries, and the serialized request.
