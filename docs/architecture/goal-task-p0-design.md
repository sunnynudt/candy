...
状态：**已确认，P0 已实现并提交**（`task_metadata` schema 17→18 + `goal.ts` 状态机 + 存储 API + 测试）。P1（续跑 policy/工具集）已在 [`goal-task-p1-design.md`](./goal-task-p1-design.md) 中实现并提交，其中按 §7.2 的“专用小方法”增补了 `setGoalNoProgress`（无 schema 变更）。
