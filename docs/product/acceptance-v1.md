...
- On the current macOS Tahoe arm64 Personal Preview host, `/access full` displays a warning and requires `/access full confirm` before Full access becomes the local default for future Auto Git tasks, including after a TUI restart. `/access safe`, `review`, and `current` turn that default off. Every resulting task records its own `full_access` decision, and the status chrome continuously shows an amber `⚠ FULL ACCESS` badge with `/access safe` as its exit. Full access retains provider-credential environment isolation, Keychain IPC denial, output redaction, process-tree cancellation, explicit recovery, and publication controls. It remains unavailable on Windows.
...
Evidence for macOS Full access: confirmed TUI warning, persisted-default restart, amber danger badge at supported terminal widths, and `/access safe` reset; outside-workspace filesystem and outbound-network fixtures; child-environment, transcript, diagnostics, and bounded-output credential negatives; disposable Keychain-canary denial; cancellation and parent-loss descendant checks; and publication-denial/recovery fixtures. Complete credential values, headers, and environment dumps are forbidden evidence.
