...
- On the current macOS Tahoe arm64 Personal Preview host, the Safe status chrome exposes a clickable `⚠ 开启 Full access` entry. It displays the `/access full` warning and requires `/access full confirm` before Full access becomes the local default for future Auto Git tasks, including after a TUI restart. `/access safe`, `review`, and `current` turn that default off. Every resulting task records its own `full_access` decision, and the status chrome continuously shows an amber `⚠ FULL ACCESS` badge with `/access safe` as its exit. Full access retains provider-credential environment isolation, Keychain IPC denial, output redaction, process-tree cancellation, explicit recovery, and publication controls. It remains unavailable on Windows.
