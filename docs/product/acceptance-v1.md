...
- `/new` is rejected while the current TUI task is running or waiting for approval; the TUI tells the user to use Esc, wait for the interrupted state, and then create the fresh task. `/new` never silently backgrounds the current foreground task.
...
- On an approved host, both writable access modes run ordinary local `npm run` commands offline without a per-command prompt, using only an already-installed selected-workspace `node_modules`; Candy never downloads dependencies implicitly. Both writable modes expose a one-command network-development approval for dependency installation, source fetches, and similar bounded work; publication and credential-bearing commands remain blocked.
- On both ENV-MAC and ENV-WIN once the platform Full Access Backend is verified, the Safe status chrome exposes a clickable `⚠ 开启 Full access` entry. It displays the warning and replaces it with a separate clickable `⚠ 确认开启 Full access` acknowledgement; these two actions are equivalent to `/access full` and `/access full confirm`. Full Access then becomes the local default for future Auto tasks, including after a TUI restart. `/access safe` turns that default off; `/access current` keeps it while working directly in the current workspace (wide sandbox + direct placement, Codex-style); `review` does not alter it. Every resulting task records its own `full_access` decision, and the status chrome continuously shows an amber `⚠ FULL ACCESS` badge with `/access safe` as its exit. Full Access runs broad local commands and outbound network operations without per-command approval, while retaining provider-credential isolation, output redaction, process-tree cancellation, explicit recovery, credential-scanned commits, explicit push authorization, and publication controls.
...
Full Access evidence is required independently on ENV-MAC and ENV-WIN: confirmed two-click Safe entry and TUI warning, persisted-default restart, amber danger badge at supported terminal widths, and `/access safe` reset; outside-workspace filesystem and outbound-network fixtures; child-environment, transcript, diagnostics, and bounded-output credential negatives; the platform credential-store canary denial; cancellation and parent-loss descendant checks; and publication-denial/recovery fixtures. Complete credential values, headers, and environment dumps are forbidden evidence.
