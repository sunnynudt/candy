...
- On the current macOS Tahoe arm64 Personal Preview host, `/access full` displays a warning and requires explicit confirmation before one Auto Git task receives broad filesystem and network command access. The persisted task evidence identifies that decision; the next task does not inherit it. Full access retains provider-credential environment isolation, Keychain IPC denial, output redaction, process-tree cancellation, explicit recovery, and publication controls. It remains unavailable on Windows.
...
Evidence for macOS Full access: confirmed TUI warning and one-task persistence; outside-workspace filesystem and outbound-network fixtures; child-environment, transcript, diagnostics, and bounded-output credential negatives; disposable Keychain-canary denial; cancellation and parent-loss descendant checks; and publication-denial/recovery fixtures. Complete credential values, headers, and environment dumps are forbidden evidence.
