...
The TUI starts in the **safe workspace** access mode: file read/create/edit/delete are enabled inside an isolated Candy-owned Task Worktree under the workspace's `.git/candy-worktrees/` directory (falling back to the Candy application-data directory when `.git` is a file, such as in submodules), and local file mutations do not pause a running task for per-operation confirmation. The resulting Git status, diff, and validation evidence remain available for task-level review. `/access` is the user-facing control: `review` is read-only, `safe` is the default isolated workflow, and `current` explicitly works in the current workspace, including existing uncommitted changes. On an approved macOS host, both writable access modes receive offline local-command capability by default: they use only the selected workspace's already-installed `node_modules`, and Candy grants the pinned Node runtime read access needed for `npm run` scripts. Candy never installs dependencies automatically; missing local dependencies remain a task-visible condition. Safe-workspace network commands still require one-command approval; current-workspace tasks do not expose network commands.

macOS Tahoe arm64 additionally exposes a Personal Preview `/access full` path. When it is available, the Safe status chrome has a clickable `⚠ 开启 Full access` entry that reveals the same warning, then changes to a separate clickable `⚠ 确认开启 Full access` acknowledgement. The two clicks match the `/access full` then `/access full confirm` shortcut, before Full access becomes the local default for future Auto tasks, including after a TUI restart. `/access safe` returns to the default sandbox and is the documented Full-access exit; `/access current` keeps the Full-access sandbox while switching tasks to the current workspace directly, giving the Codex-style wide-sandbox direct-development flow (combined with `/push allow` and model-issued `candy_git_commit` commits); `review` does not alter the Full-access default. Each Full task persists its own `full_access` evidence even when the default later changes. The status chrome continuously displays an amber `⚠ FULL ACCESS` badge with the `/access safe` escape route. Full access grants broad local filesystem and network command capability through the Native Sandbox Runner, while retaining cleared child environments, bounded/redacted output, owned process-tree cancellation, and a Keychain IPC denial for Candy provider credentials. It does not silently authorize commit, push, publish, release, or deployment; it is unavailable on Windows and is not V1 release evidence until its macOS evidence package is complete. [ADR-0013](../adr/0013-macos-full-access-personal-preview.md) is authoritative for this preview.

Credentials, Git publication, and arbitrary external roots remain blocked outside that explicit macOS Full access task. Shell and other side-effectful capabilities remain unavailable unless their platform-specific security and approval gates pass; a disabled or unverified capability is not V1 acceptance evidence.
