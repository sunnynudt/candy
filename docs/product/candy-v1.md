...
Candy is model- and provider-neutral at the product boundary. Built-in and user-configured models are ordinary selectable models with provider-specific credentials and endpoint policy. The built-in DeepSeek and MiniMax domestic Token Plan paths are supported only through their approved endpoints; MiniMax must use `https://api.minimaxi.com` and must not fail over to the global endpoint. No provider is a dedicated vision service and Candy does not silently hand image turns to another model. Image attachments are accepted only when the selected model declares image input capability. Provider model contracts, live entitlement, model capabilities, and cancellation are accepted only with the evidence described by the live-provider procedure.
