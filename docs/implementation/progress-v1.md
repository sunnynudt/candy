...
- **降级不再静默**：`/access safe` 与 `/access current` 此前无论真实能力如何都印“本地检查自动离线运行”，与本屏状态栏的“本地检查关闭”自相矛盾；启动时也不会有任何提示。现在 TUI 启动时若本地命令不可用会明确输出原因（未找到 runner / 平台门禁未过）以及后果（任务仍可读写文件，但不能跑 shell 或 git），`/access safe|current` 按真实能力输出“本地检查自动离线运行”或“本地检查不可用（原因）”；显式 `/local off` 属于用户选择，不当作故障告警。新增两个 TUI 回归测试（不可用时告警且不再声称自动离线运行、可用时保持原措辞且无告警）。
- **发行包自带 runner**：TUI 发行包此前从不包含 `candy-sandbox-runner`，安装版只能靠“从源码仓根目录启动”这一 cwd 兜底才解析得到；换到任意其它目录启动（例如用户在自己的 repo 里直接跑 `candy`）就静默变成无本地命令、无 Full access、无原生 validator（本机实测 `~/.candy/versions/*/apps/tui/dist/main.js` 在 cwd 不在源码仓时解析为 `undefined`）。`scripts/package-tui-release.mjs` 现在构建并拷贝 runner 到 `apps/tui/native/candy-sandbox-runner`（即 `resolveNativeProcessRunnerPath` 的首选候选），跨平台打包时明确报错而不是静默产出缺 runner 的包。验证：对打出的发行目录以 `cwd=/private/tmp`（源码仓之外）调用 resolver 能解析到包内 runner；对修复前的已安装目录同样调用得到 `undefined`。
...
## 2026-08-25 提示词显式化与设计取舍（`e99c88a` 之后）

- **提示词显式化（业界实践：显式指令优于重试/放宽断言）**：三个 live smoke 的任务提示词改为强制性措辞（"You MUST ..." + 明确工具顺序 + "do not skip any step"），验收契约（mjs transcript 工具断言）保持不变。改动后 journey / trusted-shell / dogfood 三个 live smoke 均一次性通过（此前 MiniMax 连续两轮跳过 list、DeepSeek 一轮跳过 bash）。journey mjs 的 prompt 恢复断言同步更新为新提示词前缀。
- **审批帧 footer 设计取舍（保留）**：帧尾 `操作：<action> · <首条详情> · /approve <id>` 摘要行与帧体存在少量内容重复；这是有意的"摘要栏"模式——它是锚点在视口尾部的确定性保障，重复成本远低于恢复视口丢行 flakiness。不做改动。
- **外部标准扫描重跑与 Apple 签名/公证**：需要外部扫描器与签名环境，本机（macOS 开发主机）无法推进，保持记录。
...
## 2026-08-19 Issue #4 macOS G2 approval and real-journey checkpoint (`2e8ee6b`)

- The product owner approved the macOS G2 gate on 2026-08-19. `MACOS_TRUSTED_SHELL_AUTO_G2_ATTESTATION.approved` is `true` (commit `926bc1a`), the composition root injects the host capability, and the fail-closed regression now asserts host-bound enablement. `npm run check` stays **248/248**.
- The Trusted Shell harness had two stale assertions relative to the current TUI: expect patterns required a closing bracket immediately after tool names even though the TUI renders bounded args, and the network-count assertion counted both `started` and `ok` transcript entries. Both were fixed (`0c78f3b`, `2e8ee6b`) without changing product code.
- Real evidence at `2e8ee6b`: `smoke:tui:trusted-shell:macos` attempts 1–3 all pass (`realProvider=true`, one network approval each, cancellation/restart/Apply verified); `smoke:tui:trusted-shell:dogfood:macos` passes `categoryScore 3/3`, `safetyFailures 0`, `offlineShellOnly=true`; `smoke:tui:terminal:macos` passes the full real-PTY matrix including Chinese UTF-8 input; `gate:live:deepseek` **7/7**; `gate:live:minimax` **8/8**; `npm run acceptance:macos` **14/14**.
- Remaining non-blocking items: independent standard-scan rerun (scanner unavailable in this session), Apple signing/notarization, and Windows 11 execution (deferred per this work packet).

## 2026-08-18 Issue #4 macOS security hardening checkpoint (`9009ac1`)

- `GitWorktreeManager` now binds the worktree root and every existing path component with no-follow `O_DIRECTORY` handles before and after each Git operation; a same-path directory replacement (different inode) or a symlink swap fails closed with `Task worktree path changed while the operation was in progress` instead of relying only on canonical strings. `nativeGitPathSeam.canonicalize` now resolves the longest existing prefix, so pre-create canonical checks do not mix `/var` and `/private/var` aliases.
- Workspace file reads/writes, `mkdir`, `access`, and delete now hold a directory-handle chain from the selected workspace root to the target parent and revalidate handle identities around the operation; a parent replacement during delete approval fails closed before `unlink`. Session directories use the same chain binding and Pi `SessionManager.create/open/listAll` calls are revalidated before and after; relative session files resolve against the Candy session root instead of the process cwd.
- `candy_bash_network` no longer runs a shell. It parses the approved command into a direct read-only tool invocation (`git ls-remote`, `curl` GET/HEAD, `wget` GET), passes `allowProcessExec: false`, and therefore cannot create publication-capable descendants. `containsShellPublicationAction` additionally rejects common nested-interpreter and executor forms (`sh -c`, `python3 -c`, `node -e`, `perl -e`, `ruby -e`, `php -r`, `env`, `xargs`, `find -exec`, `make`, `npm run`, `npx`, `sudo`, `su`, `ssh`, `docker`, and `awk system(...)`) before approval or spawn.
- New regressions: same-path parent replacement during worktree create, same-path worktree replacement during inspect, parent-directory replacement during delete approval, workspace-root replacement during delete approval, relative session-file reload inside the session root, nested-interpreter/descendant publication rejection for both shell and network tools, and read-only direct-tool network policy (including `allowProcessExec: false` assertions).
- Verification on macOS `26.6.1` arm64 with Node `22.23.2`/npm `10.9.8`: `npm run check` **248/248**, `npm run check:native` pass, `npm run smoke:sandbox:macos` strict matrix pass, `npm run acceptance:macos` **14/14** with `realPty=true`; report at `out/acceptance/macos/latest.md`. Staged diff scanned for credential-shaped material before commit; push to `origin/codex/candy-v1-foundation` verified at `9009ac1`.
- This checkpoint is implementation evidence and not a security clearance. The external standard scanner could not be rerun in this session; the independent scan gate, macOS G2 decision, current-HEAD live gates, real journeys, and final personal acceptance remain open.

## 2026-08-17 Issue #4 Windows real DeepSeek TUI journey checkpoint

- New `smoke:tui:real-journey:windows` drives the production `createDefaultInteractiveTui` composition root through the ConPTY bridge with the real Windows Credential Manager DeepSeek credential (presence-only, `candy-v1/deepseek`). The real provider turn uses the `candy_write` tool to update `src/value.ts`, the TUI projects changes and diff, and the run verifies workspace mutation, one completed task, and a credential-free fixture tree. Source revision `83d9887`, Windows 11 Pro x64, Node `22.23.2`, `realPty=true`, `realProvider=deepseek`.
- This closes the Windows live ACC-03 core coding journey evidence with a real DeepSeek request. The MiniMax Windows live Gate, the existing-account Credential Manager lifecycle, the Developer-Mode reparse matrix, independent G2, signing, and final V1 acceptance remain open.

## 2026-08-17 Issue #4 Windows ConPTY terminal matrix checkpoint

- New `smoke:tui:terminal:windows` drives the TUI through a real Windows pseudo console. The bridge host `scripts/pty-host.ps1` creates a ConPTY session (the same mechanism Windows Terminal uses) through PowerShell Add-Type P/Invoke with no third-party dependency, and spawns the TUI child with the node-pty handle pattern (`STARTF_USESTDHANDLES` with NULL handles, `bInheritHandles=false`). The parent script drives Chinese UTF-8 input, bracketed paste, `:new`, Ctrl+C, runtime-failure recovery, startup-failure safety, and `:quit` against the live TUI, then verifies task states and credential-free evidence.
- `npm run acceptance:windows` passes **17/17** with `npm run check` at **239/239 and 2 explicit skips** on Windows 11 Pro x64. The matrix child swallows a harness-only `EPIPE` during process teardown (the bridge closes the pseudo-console output pipe as its host exits; a real interactive terminal never closes it).
- This is current-host Windows real-terminal evidence. Live DeepSeek/MiniMax Windows journeys, the MiniMax Windows live Gate, the existing-account Credential Manager lifecycle, the Developer-Mode reparse matrix, independent G2, signing, and final V1 acceptance remain open.

## 2026-08-17 Issue #4 Windows cross-drive containment checkpoint

- Windows 11 Pro x64 host `npm run acceptance:windows` passes **16/16** with deterministic `npm run check` at **239/239 and 2 explicit skips**. The run covers the pinned toolchain, native check, TUI responsiveness, launcher, credential presence/revocation, Pi text/tool/coding/cancellation journeys, TUI task, Windows TUI journey, Credential Manager lifecycle, and the native fail-closed gate.
- Windows containment fix: containment checks now reject cross-drive candidates. On Windows, `path.relative` across drives returns an absolute path, which the previous `startsWith("..")`-based checks treated as inside. The fix adds `!path.isAbsolute(relative)` at the TUI app-data overlap check (`apps/tui/src/main.ts`), the restricted resource loader containment check (`packages/pi-adapter/src/restricted-resource-loader.ts`), and the Pi session-file containment check (`packages/pi-adapter/src/index.ts`).
- Windows symlink fixtures: directory links use junctions on ordinary Windows sessions; real file-symlink assertions skip explicitly on Windows without Developer Mode instead of failing with `EPERM`. The two skipped tests record the Developer Mode requirement; they are not treated as Pass.
- This is current-host Windows deterministic evidence. Live DeepSeek/MiniMax Windows journeys, the MiniMax Windows live Gate, the existing-account Credential Manager lifecycle, the Developer-Mode reparse matrix, independent G2, signing, and final V1 acceptance remain open.

## 2026-08-17 Issue #4 scope: exact macOS 26.5.2 regression baseline removed

- Per product decision, the exact macOS `26.5.2` compatibility regression baseline is removed from V1 (ADR-0012 supersedes ADR-0009). The MacBook Pro acceptance host runs macOS Tahoe `26.6.1`; `26.5.2` was the pre-upgrade system and is not a V1 target; Candy TUI only needs to run on the current macOS host.
- `npm run acceptance:macos` is now the only macOS acceptance entry point; the `--baseline` runner mode and the `npm run acceptance:macos:baseline` script are removed. macOS acceptance accepts the current Tahoe `26.x` arm64 host and records the exact version.
- Earlier entries below that list exact `26.5.2` regression as a remaining gate or record a blocked baseline preflight are historical evidence and are superseded by this decision.

## 2026-08-17 Issue #4 workspace write handle-ordering checkpoint

- `50074c2ee25ccd3d161d5096a8b6c3fce06cd6f9` is the published canonical-branch code checkpoint. `candy_write` no longer opens an existing target with `O_TRUNC` before workspace identity/canonical validation; it validates the opened regular-file handle first, then truncates and writes through that handle. This prevents the pre-validation truncate side effect while preserving normal file writes.
- Focused Pi Adapter workspace/security regressions pass **48/48**; full `npm run check` passes **241/241**. Current macOS Tahoe `26.6.1` arm64 acceptance passes **14/14**, source revision is `50074c2ee25ccd3d161d5096a8b6c3fce06cd6f9`, `realPty=true`, and the terminal matrix passes. The checkpoint is pushed and the remote SHA matches.
- Standard scan `721c4ab5-ccdf-4052-9a28-b695323b3b70` is bound to `50074c2` and still reports **4 open medium findings**: worktree path-check/use race, workspace intermediate-component race, residual Pi session-manager path race, and conditional Trusted Shell descendant-publication risk. The write ordering change is recorded as defense in depth, not as full finding closure. Windows 11, exact macOS `26.5.2`, independent G2, signing, and final V1 acceptance remain open. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 non-Git snapshot file binding checkpoint

- `88b60be367d69b1c01e7ef26c9b18a9f8e146151` is the published canonical-branch code checkpoint. Non-Git snapshot traversal now opens each regular file with a no-follow final-path handle, compares the opened identity with the pre/post lstat identity, and records size/mtime from the opened handle; any mismatch fails closed with `NonGitWorkspaceSnapshotRaceError`.
...
- Full `npm run check` passes **241/241**. Current macOS Tahoe `26.6.1` arm64 acceptance passes **14/14**, with source revision `2c4215bb4ba8be85dd8860c4f68550df4b2a11e0`, `realPty=true`, Node `22.23.2`/npm `10.9.8`, and the terminal matrix passing. The checkpoint is pushed and the remote SHA matches.
- Standard scan `b5bec00b-b140-4241-8fd2-9c8252f8ffb8` is bound to `2c4215b` and reports **5 open findings (4 medium, 1 low)**: Git worktree path-check/use race, workspace intermediate path race, residual Pi session-manager path race, non-Git file-metadata race, and conditional Trusted Shell descendant-publication risk. Parent fallback was used because the six agent slots were occupied. Windows 11, exact macOS `26.5.2`, independent G2, signing, and final V1 acceptance remain open. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 workspace binding checkpoint

- `6f63c3ee51aad91d00dd2002d13084deff573c11` is the published canonical-branch code checkpoint. Pi workspace file reads and writes now capture the selected workspace root identity, validate the opened file and canonical path after opening, and reject a changed or symlinked root; non-Git snapshot traversal rechecks each opened directory identity before accepting its metadata. A selected-root symlink regression was added.
- Full `npm run check` passes **239/239**. Current macOS Tahoe `26.6.1` arm64 acceptance passes **14/14**, with source revision `6f63c3ee51aad91d00dd2002d13084deff573c11`, `realPty=true`, Node `22.23.2`/npm `10.9.8`, and the terminal matrix passing. The checkpoint is pushed and the remote SHA matches.
- Standard scan `631e277f-7272-4d06-9d00-70f486f0a0ab` is current-HEAD bound and reports **5 open findings (4 medium, 1 low)** with parent fallback. This checkpoint reduces common final-file/root replacement routes but does not prove descriptor-relative or Windows reparse-safe behavior for directory creation, Apply/worktree, session-manager operations, or file-level snapshot metadata. Windows 11, exact macOS `26.5.2`, independent G2, signing, and final V1 acceptance remain open. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 current-HEAD security scan refresh

- Standard scan `2c682ace-5786-4844-9890-1eb2052b6a38` reviewed the current source snapshot at `09dc079` and completed with **5 open findings: 4 medium, 1 low**. The independent baseline/investigator workers could not start because all six subagent slots were occupied; the parent performed a focused source validation and marked coverage partial.
- Remaining findings are Apply/worktree replacement races, workspace-operation replacement races, residual session-root path races after pre-existing symlink hardening, non-Git snapshot traversal races, and Trusted Shell publication bypass through interpreter-created descendants. The report is evidence for remaining work, not a security clearance.
- The scan explicitly excludes `.omo/**` as user-owned untracked content. Windows runtime behavior, exact macOS `26.5.2`, platform G2, signing, and final V1 acceptance remain separate open gates.

## 2026-08-17 Issue #4 Pi session-root symlink checkpoint

- `e6170b6782908aa29b8a43337b3e6cffa67d1f38` is the published canonical-branch code checkpoint. Candy now prepares `sessionRoot/taskId` and `sessionRoot/pi-agent` with final-directory symlink rejection; reload/recovery and selected existing Pi sessions validate regular files and read through no-follow handles before Pi opens them. The macOS `/var` to `/private/var` system alias remains supported because validation is scoped to Candy's session root and its children.
- Focused Pi adapter tests pass **47/47**; full `npm run check` passes **238/238**. The post-push macOS Tahoe `26.6.1` arm64 acceptance passes **14/14**, with source revision `e6170b6782908aa29b8a43337b3e6cffa67d1f38`, `realPty=true`, and the terminal matrix passing.
- This closes the pre-existing symlink route covered by the new regression, not all descriptor-relative or OS-level replacement races. Workspace/apply/worktree races, residual session/non-Git race analysis, nested-interpreter Trusted Shell publication policy, Windows 11 acceptance, exact macOS `26.5.2`, independent G2 approval, signed release, and final V1 acceptance remain open. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 non-Git snapshot symlink checkpoint

- `1bcdd9f2fd51bd97d45b9f9279ad869e472c5f2f` is the published canonical-branch code checkpoint. Non-Git workspace snapshots now lstat the root and enumerated children, reject symlinks and non-regular entries, and avoid descending into symlinked directories; a regression covers pre-existing symlink entries and outside-root content.
- Focused runtime tests pass **31/31**; full `npm run check` passes **237/237**. The post-push macOS Tahoe `26.6.1` arm64 acceptance passes **14/14**, with source revision `1bcdd9f2fd51bd97d45b9f9279ad869e472c5f2f`, `realPty=true`, and the terminal matrix passing.
- This closes the ordinary symlink-follow route covered by the new regression, not all descriptor-relative or OS-level replacement races. Workspace/apply/worktree/session races, residual non-Git traversal race analysis, nested-interpreter Trusted Shell publication policy, Windows 11 acceptance, exact macOS `26.5.2`, independent G2 approval, signed release, and final V1 acceptance remain open. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 attachment-source checkpoint

- `7489139e07dfcea241fa421144541df6e2c06c39` is the published canonical-branch code checkpoint. TUI attachment import now uses an opened file handle with a platform-aware no-follow final flag, validates the opened object as a regular file, enforces `MAX_ATTACHMENT_BYTES` from the handle metadata, and reads the bytes through that handle rather than reopening the path with `readFile`.
- Focused TUI tests pass **39/39**; full `npm run check` passes **236/236**. The post-push macOS Tahoe `26.6.1` arm64 acceptance passes **14/14** and its report is source-bound to `7489139e07dfcea241fa421144541df6e2c06c39`.
- This closes only the inspected-before-read attachment-source finding from the pre-checkpoint scan. Descriptor-relative/reparse-safe workspace, Apply/worktree, session-root, and non-Git snapshot races; nested-interpreter Trusted Shell publication policy; Windows 11 acceptance; exact macOS `26.5.2`; independent G2 approval; signed release; and final V1 acceptance remain open. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 native request-boundary checkpoint

- `e7ba88b30531a1fb3c9af2845492cf307788750a` is the published canonical-branch code checkpoint. The Node native-process adapter now calculates a bounded JSONL request size before full `JSON.stringify` materialization, rejects oversized args/paths/environment before spawn, and retains pre-serialization credential rejection semantics.
- The new spawn-not-reached regression and existing nested-secret/deepest-representation coverage pass; `npm run check` passes **236/236**, native focused tests pass **14/14**, and current macOS Tahoe `26.6.1` arm64 acceptance passes **14/14** with source revision `e7ba88b`.
- This checkpoint addresses the native request materialization finding in the older scan snapshot. It does not close descriptor-relative/reparse-safe filesystem races, nested-interpreter Trusted Shell publication policy, Windows 11 acceptance, exact macOS `26.5.2`, independent G2 approval, signed release, or final V1 acceptance. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 provider-stream cancellation checkpoint

- `23b3f67ad248cdf1ba516fd953a609e725b09e13` is the published canonical-branch code checkpoint. DeepSeek and domestic MiniMax SSE readers now cancel promptly on an abort signal, close the underlying body in cleanup, and reject after cancellation instead of treating a cancelled `done` read as successful completion.
- Regression coverage uses real pending `ReadableStream` readers for both providers. `npm run check` passes **235/235**, native checks pass, and the Pi cancellation smoke proves `providerAbortObserved=true`, `taskState=cancelled`, `falseCompletion=false`, and preserved transcript evidence.
- Current macOS Tahoe `26.6.1` arm64 acceptance passes **14/14** with source revision `23b3f67`; current-source live DeepSeek passes **7/7** and domestic MiniMax passes **8/8**. Reports are credential-sanitized.
- This checkpoint addresses the provider-stream cancellation finding in the older scan snapshot; it does not close descriptor-relative/reparse-safe filesystem races, nested-interpreter Trusted Shell publication policy, Windows 11 acceptance, exact macOS `26.5.2`, independent G2 approval, signed release, or final V1 acceptance. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 TUI evidence-sanitization checkpoint

- `8f3bf78948fa68e2c854ec27a87446aad11617ea` is the published canonical-branch code checkpoint. The interactive TUI now strips ANSI/OSC/other terminal control sequences from assistant evidence while preserving newlines; the regression verifies sanitized transcript persistence and rendered content. `npm run check` passes **233/233**.
- Current macOS Tahoe `26.6.1` arm64 acceptance passes **14/14** and both current-source live provider gates pass: DeepSeek **7/7** and domestic MiniMax **8/8**. The reports are credential-sanitized and source-bound to `8f3bf78`.
- This checkpoint does not close the fresh scan's descriptor-relative/reparse-safe race findings, nested-interpreter Trusted Shell publication policy, Windows 11 acceptance, exact macOS `26.5.2`, independent G2 approval, signed release, or final V1 acceptance. Desktop and Browser remain V2. `.omo/` remains user-owned and untracked.

## 2026-08-17 Issue #4 attachment-security checkpoint

- `c6eb334d6578d5db5d76a672339a05d12b3b3cf5` is the published canonical-branch code checkpoint. It adds exclusive/no-follow attachment metadata creation, regular-file validation on metadata reads, duplicate-put compatibility, and a pre-existing metadata-symlink regression. Current macOS Tahoe `26.6.1` arm64 acceptance passes **14/14**, embedded `npm run check` passes **232/232**, DeepSeek passes **7/7**, and domestic MiniMax passes **8/8** at `https://api.minimaxi.com`.
- The current acceptance and provider reports are all source-bound to `c6eb334`; the report artifacts are sanitized and ignored. `.omo/` remains unstaged and untracked.
- Fresh scan `76445169-b4e0-4e89-9f30-a22cf156b5d9` was bound to the preceding docs-only `2911fcd` snapshot and reported 9 open findings. The attachment metadata finding is addressed by this checkpoint; descriptor-relative filesystem TOCTOU, nested-interpreter Trusted Shell publication indirection, and other same-user race risks remain open.
- Current evidence remains macOS-only and does not prove Windows 11, exact macOS `26.5.2`, signed release, independent Trusted Shell/G2 approval, or final V1 acceptance. Desktop and Browser remain V2.

- Historical checkpoint `7aa382c2330e5c62acd51cfa97c176c74d81938f` is retained below for evidence history. Its prior 220/220 and 7/8 figures are superseded by the current `bd8428b` results above.
...
Current Issue #4 security boundary: credential/provider/publication and the reviewed path categories above have source-backed fixes and regressions. Trusted Shell is now fail-closed at the normal production composition roots pending independent G2 approval. These are implementation results, not independent G2 approval; residual OS-level containment, platform-specific reparse/race evidence, and the current-HEAD platform evidence remain open.

可持续更新的实施待办见 [Candy V1 待办与进度](todolist-v1.md)。其中 `☑️` 仅表示对应范围已经验证完成，`◐` 会明确列出剩余验收条件。

## Current Issue #4 scope status

Post-`b672d5a` current-host revalidation: `npm run acceptance:macos` passes **14/14** on macOS `26.6.1` arm64, with source revision `b672d5a7e0f08c6b5dfe6f1c8299c88be79f40f2`, Node `22.23.2`/npm `10.9.8`, and deterministic `npm run check` at **213/213**. The exact `npm run acceptance:macos:baseline` command is blocked because this host is `26.6.1`, not `26.5.2`; the baseline report records that preflight result. The earlier figures below are historical checkpoint evidence.
...
Post-`236b2f2` current-host revalidation: `npm run acceptance:macos` passes **14/14** on macOS `26.6.1` arm64, with deterministic `npm run check` at **204/204**. The report also covers the TUI `:resources` diagnostics path for malformed and conflicting Candy-owned resources.

Post-compaction-cancellation evidence: `npm run check` passes **205/205**; the new TUI regression cancels after compaction starts without settled or completed projection.

Post-production-Pi-compaction evidence: `npm run check` passes **207/207**; deterministic Pi Adapter fixtures now cover context overflow through compaction and continuation, plus cancellation while the compaction provider request is in flight.
...
Post-clean-install evidence: on the pinned Node `22.23.2`/npm `10.9.8` toolchain, `npm ci --ignore-scripts` installed the committed lockfile graph without changing `package-lock.json`, reported 0 vulnerabilities, and was followed by successful toolchain assertion and build.

Issue #4 is the current delivery boundary. V1 is the macOS/Windows 11 TUI only; Desktop, Browser Workspace, cross-client handoff, and their dependent long-running workflows are V2 and must not block the TUI release decision. For this work packet, the user has explicitly deferred Windows execution and narrowed personal Preview acceptance to macOS `26.6.1`; that is not a Windows or cross-version acceptance claim. The current code checkpoint is `b0a5e30`; its macOS acceptance is separately source-bound to `b0a5e30` and passes 14/14 on macOS `26.6.1` arm64 with Node `22.23.2`/npm `10.9.8`, deterministic checks 241/241, bounded credential-redacted tool lifecycle evidence, presence-only credential and revocation evidence, Pi-backed projection/persistence/workspace mutation/review-restart/cancellation evidence, the TUI-only terminal matrix, and `realPty=true`. DeepSeek live gate 7/7 and domestic MiniMax live gate 8/8 are source-bound to `a523aa2` and retain only sanitized evidence. The latest standard scan `721c4ab5-ccdf-4052-9a28-b695323b3b70` remains bound to the prior production code checkpoint `50074c2` and reports four medium residual findings; the later checkpoint only fixes the macOS smoke fixture and is not a security clearance. macOS G2, signing, and final personal acceptance remain open. Windows evidence remains separately pending and is not substituted by macOS evidence.

| Phase                           | Status      | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Remaining                                                                                                      |
| ------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
| 0. Compatibility Gate           | In progress | Current source `b0a5e30` passes the pinned Node/npm toolchain assertion, deterministic checks 241/241, current macOS TUI acceptance 14/14, and the current macOS strict containment matrix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | macOS G2, remaining native/security verification, and personal release evidence                                |
...
| 2. Runtime proof                | In progress | Pi public `AgentSession` path runs with a temporary in-memory credential store, read-only Pi tool allowlist, streamed deltas, Candy-owned session JSONL, secret-free session assertion, retry/compaction/cancellation fixtures, current TUI Pi journeys, and source-bound live DeepSeek 7/7 plus domestic MiniMax 8/8 gates                                                                                                                                                                                                                                                                                                                                                                                                          | macOS G2 and final product-owner acceptance                                                                    |
| 3. Task Runtime and TUI         | In progress | Task lifecycle, revision fencing, FIFO scheduler, durable queued reorder, leases, approval profiles, read-only runtime, mutation serialization, SQLite metadata, TaskController recovery, interactive TUI prompt/task/cancel/reprioritize loop, explicit file Auto with read/create/edit/delete, per-delete approval, model persistence, continuous same-task turns, durable TUI transcript projection, workspace changes/diff review, explicit native validator control, explicit model/attachment controls, provider-failure recovery hints, durable app-server create/run/pause/resume/cancel/reorder event loop, and a macOS Trusted Shell Auto implementation path that is now source-gated off pending independent G2 approval | Windows/Desktop parity, Shell-based Auto Debug, and full ownership/recovery matrix                             |
| 4. Models and attachments       | In progress | Code-owned DeepSeek Flash/Pro and domestic MiniMax catalog, explicit TUI model aliases and persistence, Pi-backed MiniMax M3 typed image turns, validated Candy-owned image storage, path/symlink/workspace/app-data/size/MIME/corruption rejection, attachment-id restart recovery, domestic endpoint, no-fallback policy, and source-bound live DeepSeek/MiniMax gates pass                                                                                                                                                                                                                                                                                                                                                        | macOS G2 and personal release evidence                                                                         |
| 5. Desktop (V2)                 | Deferred    | Historical implementation evidence remains below; Desktop is outside the current TUI V1 delivery line                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Separate V2 scope and acceptance contract                                                                      |
...
| 7. Browser Workspace (V2)       | Deferred    | Historical implementation evidence remains below; Browser Workspace is outside the current TUI V1 delivery line                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Separate V2 scope and acceptance contract                                                                      |
| 8. Long-running/Auto Debug (V2) | Deferred    | Historical implementation evidence remains below; the full workflow is outside the current TUI V1 delivery line                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Separate V2 scope and acceptance contract                                                                      |
| 9. Release hardening            | In progress | Current code `b0a5e30` passes `npm run check` 241/241, source-bound macOS acceptance 14/14, current strict containment matrix, and live DeepSeek/MiniMax provider gates; workspace/worktree/session hardening checkpoints, credential/provider gates, stable `candy` bin, and TUI-only scope are recorded; standard scan `721c4ab5` reports four medium residual findings                                                                                                                                                                                                                                                                                                                                                            | Descriptor/reparse-safe residual security work, macOS Trusted Shell G2, signing, and final personal acceptance |

## 2026-08-17 Issue #4 credential revocation checkpoint

- Checkpoint `34ae949` adds `smoke:tui:credential-revocation`. It uses an in-memory Candy credential store, the production `PiAgentEngine`, the real TUI control loop, and deterministic mocked DeepSeek SSE; it never mutates the user's Keychain. After a successful real provider turn, TUI deletion removes the credential presence, and the next provider operation returns `needs_credentials` without issuing another provider request.
- The smoke also proves prior history remains preserved and the Candy-owned session remains credential-free. The current macOS Tahoe `26.6.1` arm64 acceptance passes **12/12** with `npm run check` at **197/197**, including credential presence-only, revocation, Pi text/tool journeys, the TUI journey, and the real PTY terminal matrix.
- This is current-host TUI evidence only. Windows 11 x64 acceptance, exact macOS `26.5.2` regression, independent platform Trusted Shell G2, live provider cancellation, signed release, and final V1 acceptance remain open.

## 2026-08-17 Issue #4 Pi coding review/restart checkpoint

- Checkpoint `a36aac4` persists bounded, sanitized workspace review metadata in Candy-owned SQLite. The TUI now restores the review state across restart, so a reviewed Git Task Worktree can still be explicitly Applied after reopening; starting a new continuation clears stale review metadata.
- `smoke:tui:pi:coding` drives the production `PiAgentEngine` through `candy_write`, `:changes`, `:diff`, TUI restart, `:transcript`, and `:apply`. It proves two approved DeepSeek requests, task/transcript/tool evidence persistence, workspace mutation, unchanged Git HEAD/index, and a credential-free Pi session.
- Current macOS Tahoe `26.6.1` arm64 acceptance passes **13/13** with `npm run check` at **199/199**. This is current-host TUI evidence only; Windows 11, exact macOS `26.5.2`, platform Trusted Shell G2, live provider cancellation, signed release, and final V1 acceptance remain open.

## 2026-08-17 Issue #4 Pi provider cancellation checkpoint

- Checkpoint `54b54c1` adds `smoke:tui:pi:cancellation`, using the production `PiAgentEngine` and real TUI control loop with a deterministic DeepSeek SSE stream. The smoke sends partial provider output, issues `:cancel`, observes the provider `AbortSignal`, and verifies the task is `cancelled` without false completion.
- The cancellation smoke also proves partial assistant transcript preservation, exactly one approved domestic DeepSeek request, and a credential-free Candy-owned session. Current macOS Tahoe `26.6.1` arm64 acceptance passes **14/14** with Node `22.23.2`/npm `10.9.8` and `npm run check` at **199/199**.
- This is current-host TUI evidence only. Windows 11, exact macOS `26.5.2`, platform Trusted Shell G2, live-provider cancellation, signed release, and final V1 acceptance remain open.

## 2026-08-17 Issue #4 Candy prompt invocation checkpoint

- Checkpoint `961537a` adds `:prompts` and `:prompt <name> [args]` to the Interactive TUI. Templates are loaded only through the Candy restricted resource loader from Candy-owned app-data `prompts/*.md`; `$1..$99`, `$ARGUMENTS`, and `$@` substitutions support quoted arguments, while unknown templates, malformed arguments, control characters, oversized arguments, empty templates, and active credential material fail closed.
- The public TUI regression covers listing, quoted-argument expansion, active-secret redaction before engine submission, unknown-template rejection, and the bounded argument guard. `npm run check` passes **201/201** on the pinned Node `22.23.2`/npm `10.9.8` toolchain.
- This is current-host deterministic TUI evidence. Windows 11, exact macOS `26.5.2`, live-provider revalidation on this new revision, platform Trusted Shell G2, signing, and final V1 acceptance remain open.

## 2026-08-17 Issue #4 lifecycle projection checkpoint

- Checkpoint `a28606c` makes successful provider retries visible in the TUI (`provider retry N succeeded`) and retains the ordered retry, compaction started/completed, assistant output, `turn settled`, and final completion projection.
- The Pi Adapter now fails closed before acquiring a credential lease or starting a provider/session when its abort signal is already cancelled. TUI and adapter regressions cover retry/compaction ordering and pre-cancel behavior; `npm run check` passes **203/203**.
- Current macOS acceptance remains **14/14** on Tahoe `26.6.1` arm64. This does not substitute for Windows 11, exact macOS `26.5.2`, or live-provider revalidation on the new revision.

## 2026-08-17 Issue #4 resource diagnostics checkpoint

- Checkpoint `236b2f2` adds `:resources` to the TUI. It reports only bounded category/type/message/path diagnostics from Candy-owned skills and prompt templates; resource contents, credentials, and arbitrary executable resources remain outside the display path.
- The regression covers a malformed skill frontmatter error and a duplicate prompt-name collision. `npm run check` passes **204/204**, and current macOS acceptance passes **14/14**.
- This closes the deterministic user-facing diagnostics gap for Issue #4 personal resources. Windows 11, exact macOS `26.5.2`, live-provider revalidation on this revision, G2, signing, and final V1 acceptance remain open.

## 2026-08-17 Issue #4 compaction cancellation evidence checkpoint

- The TUI regression now holds a turn after `context compaction: overflow`, cancels it through the public `:cancel` command, and proves the task reaches `cancelled` without a compaction-settled, turn-settled, or false-completed projection.
- This is deterministic TUI control-loop evidence. It does not replace production Pi compaction fixtures, Windows 11 execution, exact macOS `26.5.2`, platform Trusted Shell G2, signing, or final V1 acceptance.

## 2026-08-17 Issue #4 production Pi compaction checkpoint

- The Pi Adapter fixtures now drive the production `PiAgentEngine` through a real context-overflow path: a large persisted turn triggers Pi compaction, the summary request completes, and the turn continues with recovered assistant output. A second fixture cancels while the compaction provider request is in flight and confirms the turn rejects as cancelled without `turn.completed`.
- With the pinned Node `22.23.2`/npm `10.9.8` toolchain, `npm run check` passes **207/207**. This is deterministic production-Pi evidence; it does not close Windows 11, exact macOS `26.5.2`, platform Trusted Shell G2, live-provider revalidation on this revision, signing, or final V1 acceptance.

## 2026-08-17 Issue #4 live provider revalidation checkpoint (source `7f2d967`)

- The Candy-owned DeepSeek live Gate passes **7/7** against `https://api.deepseek.com`, covering Flash/Pro streaming and tool replay, cancellation, controlled error contracts, secret-free session scanning, and lease release.
- The domestic MiniMax live Gate passes **8/8** against `https://api.minimaxi.com`, covering text, image, thinking/tool replay, cancellation, controlled error contracts, product-policy `LIVE-MM-05`, secret-free session scanning, and lease release.
- Both reports are sanitized and bound to source revision `7f2d967`; they do not close Windows 11, exact macOS `26.5.2`, platform Trusted Shell G2, signing, or final V1 acceptance.

## 2026-08-17 Issue #4 historical live provider checkpoint (source `b23383a`)

- On source revision `b23383a`, `npm run gate:live:deepseek` passes **7/7** against the approved `https://api.deepseek.com` endpoint, including streaming, tool/replay, cancellation, controlled error contracts, secret-free session scan, and lease release.
- On the same revision, `npm run gate:live:minimax` passes **8/8** against the domestic `https://api.minimaxi.com` endpoint, including text, image, thinking/tool replay, cancellation, controlled error contracts, product-policy `LIVE-MM-05`, secret-free session scan, and lease release.
- Reports are sanitized and local under `out/acceptance/live/`; no credential value, fingerprint, header, prompt, or raw provider payload is retained. These are current macOS provider-path gates, not Windows 11, exact macOS `26.5.2`, G2, signing, or final V1 acceptance evidence.

## 2026-08-17 Issue #4 current macOS native containment revalidation checkpoint (source `38775e2`)

- `npm run smoke:sandbox:macos` passes the strict matrix: Candy workspace guards reject outside and symlink reads/writes; the native runner rejects outside/symlink/symlink-swap access, Git metadata/ref/reflog writes, and default loopback networking while allowing only explicit network capability; ordinary, detached, and parent-exit descendant cleanup all pass.
...
- `npm ci --ignore-scripts` completed on the pinned Node `22.23.2`/npm `10.9.8` toolchain, installed the committed dependency graph, left `package-lock.json` unchanged, and reported 0 vulnerabilities; `npm run check:toolchain` and `npm run build` passed afterward.
- This closes current-host deterministic ACC-TUI-01 install/build evidence only; Windows 11, exact macOS `26.5.2`, G2, signing, and final V1 acceptance remain separate.
...
## 2026-08-14 Issue #3 macOS Trusted Shell Auto Personal Preview checkpoint

- Source revision `9c5afadb74895de9c688445b4889773de873d34a` enables Trusted Shell Auto only from the normal macOS arm64 TUI composition root. The user must still select Auto, explicitly enable Trusted Shell, and use a Git-backed Candy Task Worktree; Desktop, Windows, and Shell-based Auto Debug remain disabled or separately gated.
- The completed standard security scan covers 168 repository files and reports **0 reportable findings** for the Issue #3 boundary: task/worktree ownership, Seatbelt containment, offline shell default, one-command network elevation, publication denial, credential sinks, attachment retrieval, app-server/Desktop rejection, and TUI recovery.
- Current macOS Tahoe `26.6.1` arm64 evidence on this revision: `npm run acceptance:macos` **21/21**, `npm run check` **178/178**, Rust tests **7/7**, real DeepSeek live gate **7/7**, three real Provider/Pi TUI journeys pass, dogfood **3/3** with zero safety failures, and real PTY terminal matrix pass. Evidence is credential-free and keeps Git HEAD/index unchanged.
- This closes the macOS Personal Preview implementation/evidence slice only. It does not claim Windows 11, exact macOS `26.5.2`, signed/notarized packaging, Desktop Shell, or final V1 acceptance.

## 2026-08-14 macOS Trusted Shell Auto Personal Preview checkpoint

- ADR-0010 supersedes the platform-enablement consequence in ADR-0005: the macOS TUI has an implementation path for Trusted Shell Auto as a separately gated Personal Preview, but the normal composition root keeps the capability flag off until macOS G2 review; Windows remains disabled until its own native G2 gates pass.
- TUI capability selection is explicit (`:trusted-shell on|off`, with `:shell` as an alias), requires the Auto profile and a Git-backed Candy Task Worktree, and persists `trusted_shell` on the task. Read-only tasks, non-Git tasks, and Local Workspace execution never receive the shell tool.
- Ordinary `candy_bash` calls run through Pi's public Bash definition and Candy's native runner with `network: false` and no per-command approval. `candy_bash_network` is a separate one-command tool: it validates a bounded command/reason/timeout, displays the canonical Worktree cwd, waits for one TUI approval, and passes `network: true` only to that approved run. Denial, cancellation, TUI exit, and owner loss do not spawn or replay a command.
- The native JSONL contract defaults an omitted network field to offline. macOS Seatbelt uses default-deny workspace containment plus an explicit `network-outbound` allowance only for an elevated request; Windows rejects network capability before launch. Provider credentials remain excluded from shell environments, native requests, approval text, and bounded output.
- Verification: `npm run check` passes **171/171**; `cargo test --locked` in `native/sandbox-runner` passes **7/7**; the macOS smoke matrix additionally proves a real linked Git Worktree status read, Git HEAD/ref/reflog metadata write rejection, and TLS CA/config access only on the explicitly network-elevated native profile. The real terminal matrix passes on evidence-producing revision `fb400aa` on macOS Tahoe `26.6.1` arm64 with Chinese UTF-8 input, bracketed paste, resize, startup/runtime-failure recovery, and Ctrl+C cancellation, with credential-free temporary PTY/app-data/Worktree evidence. Three clean real-PTY journeys (attempts 1/2/3) use production Candy Pi Agent Engine + DeepSeek Flash and pass offline Shell Auto, one-command `git ls-remote` approval, validator, Apply, cancellation, restart recovery, unchanged Git HEAD/index, and credential-free evidence; each journey scans the complete temporary fixture root, including the Task Worktree. The separate dogfood journey passes repository understanding, small repair, and failing-test diagnosis **3/3** with zero safety failures. The default TUI composition-root test also verifies that Trusted Shell remains rejected until the macOS G2 gate is explicitly enabled. Reports are sanitized under `out/acceptance/macos/` and not committed. Exact `26.5.2` regression, independent G2 security review, and final V1 acceptance remain separate evidence gates.
- The TUI recovery seam now explicitly settles a waiting network approval on TUI exit, rejects the old approval id after restart, and interrupts a dead-owner waiting task without replay. These are deterministic ownership/recovery checks; they do not replace the independent G2 decision or Windows evidence.

## 2026-08-14 Trusted Shell credential-redaction follow-up

- Published revision `aea91a6d3fcd9a0eb09ea2dac2460808747dd1b7` closes the selected-provider-only shell redaction gap: the TUI keeps all active Candy provider leases alive for the full Trusted Shell turn, passes the complete active secret set to the Pi adapter, and the adapter deduplicates it with the selected lease before Native Runner redaction. A focused regression covers active DeepSeek and MiniMax secrets without exposing their values.
...
- `NativeProcessRunner` now includes the Candy parent PID in the typed native request. On macOS, the Rust runner puts the sandboxed command in its own process group, catches supervisor cancellation, and starts a detached reaper that watches the Candy parent independently. The reaper terminates the owned group on parent loss and tracks/reclaims detached descendants after normal completion. The omitted/zero PID remains accepted only for direct protocol fixtures; the TypeScript caller always sends the live PID.
- The macOS strict-containment smoke now covers ordinary cancellation, detached-descendant cleanup after normal Trusted Shell completion, and a separate launcher-kill scenario in which a delayed descendant must not write a marker after its Candy parent exits. Native output remains bounded and credential checks remain at the TypeScript/Rust protocol boundary.
- Validation on Node `22.23.2`/npm `10.9.8`: `npm run check` passes **176/176**; `cargo fmt -- --check`, `cargo test --locked` (**7/7**), and `cargo build --locked` pass; `npm run smoke:sandbox:macos` passes the full strict-containment matrix including `parentExitLauncherKilled` and `parentExitMarkerAbsent`. This advances implementation evidence only; independent G2, Windows, exact `26.5.2`, signed packaging, and final V1 acceptance remain open.
- After publishing evidence-producing revision `fb400aac4e6570e567e95daeb36ce1ff72cf7ec0`, the real-PTY matrix passed with Chinese UTF-8 input, bracketed paste, resize, startup/runtime failure recovery, and Ctrl+C cancellation. This does not by itself prove an OS-level IME path. Trusted Shell attempts `1/2/3` each passed the real DeepSeek/Pi coding journey with offline Shell Auto, one bounded network approval, validator, Apply, cancellation, restart recovery, unchanged Git HEAD/index, and credential-free evidence; the separate dogfood run passed repository understanding, small repair, and failing-test diagnosis `3/3` with zero safety failures. These are current macOS `26.6.1` arm64 evidence only and do not clear independent G2 or other platform/release gates.
- Follow-up revision `fd7f6587cbe7acb27c60cf50a02d44a4e4ce94ad` permits only metadata traversal under canonical macOS `/private/tmp` so a clean acceptance environment without `TMPDIR` still reaches its own workspace; it does not grant file read/write outside the workspace. The current macOS acceptance runner then passed **21/21** deterministic and packaged steps on macOS `26.6.1` arm64, including the parent-exit cleanup smoke. The report remains non-live-provider, unsigned, and not a final V1 or independent G2 decision.
...
- The sanitized report `out/acceptance/live/deepseek-latest.md` records `7 passed, 0 failed, 0 blocked`: `LIVE-DS-01` Flash text, `LIVE-DS-02` Flash tool replay, `LIVE-DS-03` Pro thinking/tool replay, `LIVE-DS-04-cancel`, controlled 401/429/timeout error contracts with recovery, `secret-free-session`, and `secret-lease-release`. Flash and Pro reasoning-delta obligations are intentionally distinct because the two models expose them differently.
