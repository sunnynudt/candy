...
- `npm run acceptance:windows` ran on clean source revision `98f7723285fbb810a316842605ef35f820a69d4d` with Node `22.23.2`, verified Electron `43.2.0`, and the current Windows x64 host. All 11 deterministic steps passed: toolchain, full TypeScript check (93 tests), native check, Windows Job Object smoke, Credential Manager fixture, TUI task smoke, app-server smoke, process-restart recovery smoke, development Desktop smoke, unsigned packaged Desktop smoke, and the ten-run responsiveness subset. The latest p95 values are TUI `1355 ms` and Desktop `1592 ms`.
