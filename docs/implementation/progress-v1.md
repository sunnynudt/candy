...
- On macOS `26.5.2` arm64 with Node `22.23.2` and Electron `43.2.0`, the full `acceptance:macos` run passed 15/15 steps and the sanitized report `out/acceptance/macos/responsiveness-latest.md` records TUI/Desktop cold-start p95 `844/1555 ms`; Runtime projection p95 `1 ms` (target `<=200 ms`), process-tree termination p95 `12 ms` (target `<=5000 ms`), Take Control rejection p95 `3 ms` (target `<=500 ms`), and concurrent renderer frame-gap p95 `19 ms` (target `<=1000 ms`). All ten concurrency runs delivered `9/9/9=>9/9/9` projections with no event-loss run.
