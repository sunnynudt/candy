...
| 5. Desktop                  | In progress | Versioned JSONL app-server, task event projection, explicit model selection, transcript/diff shell, Electron 43.2.0 secure window/preload/IPC seam, Candy browser partition, credential write-only bridge, close-vs-quit policy, and macOS arm64 packaged smoke pass | Apple-signed child runtime, Sequoia evidence, OS credential acceptance, packaged Browser evidence |
...
- The current M2 Pro development machine is macOS `26.5.2` arm64, not the exact Sequoia 15 acceptance environment.
