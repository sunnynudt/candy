...
- `npm run check` passes `159/159`, including real Git Apply/Discard and active-secret fixtures; the current macOS real-PTY TUI journey also passes. Shell and Shell Auto Debug remain disabled pending G2, and this checkpoint is not Windows, live-provider, signed-package, or final V1 evidence.
