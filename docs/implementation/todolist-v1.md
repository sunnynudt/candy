...
- ☑️ TUI Git Task Worktree / Apply / Discard 已接入：Auto Git 任务先在 Candy app-data 下创建并持久化锁定 Worktree，Pi turn、恢复、validator 和 diff 均不直接改 Local；`:apply` 仅在 completed/ownerless 且用户依次查看当前完整 `:changes` 与未截断全量 `:diff`（含 untracked 文本内容；untracked binary fail-closed）后可用，并在 transfer 前重新检查/指纹比对；`:discard` 只清理 Candy-owned Worktree。Apply 同时阻断 active DeepSeek/MiniMax credential 出现在路径、tracked patch 或 untracked 文件字节中。根目录新增 `npm run tui`；`npm run check` 159/159 和当前 macOS real-PTY journey 通过。Shell/Auto Debug 仍等待 G2，未据此宣称 Windows、签名或最终 V1 Pass。
