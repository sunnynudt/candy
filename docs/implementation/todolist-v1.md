...
当前基线提交：`f543b9f6ba9b6159156ba2a4d74a8931f3406227`
...
- ◐ 完成 Windows Credential Manager、桌面打包、签名、恢复和响应性矩阵。
  ...
  待完成：DeepSeek Candy 账户本次已存在，smoke 未触碰该真实账户；对该固定账户补做替换/删除需要用户授权的可恢复测试窗口或专用空账户。Windows 正式安装/下载、签名、packaged active-owner/tool recovery 和完整响应性矩阵仍未完成；active validator interruption、owner crash interruption、non-owner read-only fencing、attachment restart recovery 和 bounded three-slot concurrency 已有本机 smoke 证据。该开发态 smoke 不替代 Windows 签名、恢复或完整 Desktop 验收。

- ☑ 重新生成当前提交的 Windows 验收证据。
  已完成：在干净提交 `f543b9f6ba9b6159156ba2a4d74a8931f3406227` 上执行 `npm run acceptance:windows`，16/16 个确定性步骤通过，包含 queued/active crash recovery、attachment restart recovery、active validator interruption、non-owner read-only fencing、bounded three-slot concurrency、未签名 packaged Desktop、扩展后的 Browser action/security fixture 和 packaged Credential Manager fixture smoke；脱敏报告为 `out/acceptance/windows/latest.md`，由 `.gitignore` 排除。
  ACC-12 确定性 cold-start 子集本次 p95 为 TUI `1206 ms`、Desktop `1388 ms`；并发槽位/FIFO 子集通过，但完整 ACC-12 指标仍未完成。
