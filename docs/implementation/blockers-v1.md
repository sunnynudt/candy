...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | The clean-revision Windows deterministic runner passes 10/10 steps on the current Windows 11 Pro x64 host, including queued task metadata process-restart recovery; continue the dedicated checklist for active-owner/tool recovery, signed/packaged Desktop, Browser, complete G2, remaining ACC metrics, and external provider/signing gates. |
