...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | The clean-revision Windows deterministic runner passes the current 14-step set on the Windows 11 Pro x64 host, including active validator interruption, queued/active process-restart recovery, non-owner read-only fencing, unsigned packaged Desktop JSONL smoke, packaged Browser action/security fixture smoke, and packaged Credential Manager fixture smoke; continue the dedicated checklist for attachment/packaged recovery, signed installation, complete Browser/adversarial coverage, complete G2, remaining ACC metrics, and external provider/signing gates. |
...
- TUI and signed Desktop installation, packaged Node app-server lifecycle, and the remaining full ACC-12 responsiveness matrix. The unsigned Windows package and Browser action/security fixture now pass their deterministic subset; queued/active owner crash recovery and active validator interruption pass; the latest clean-revision ten-run subset has TUI p95 `1391 ms` and Desktop p95 `1459 ms`; cross-client handoff, Browser physical input-origin/adversarial coverage, concurrency, and signed installation evidence remain open.
