...
Post-`2e8ee6b` current-host revalidation: macOS acceptance passes **14/14** with embedded deterministic `npm run check` at **248/248** on Node `22.23.2`/npm `10.9.8`, and the current strict macOS containment matrix passes. The four medium findings from standard scan `721c4ab5-ccdf-4052-9a28-b695323b3b70` are closed at the source level by code checkpoints `9009ac1` + `3408413` (worktree/workspace/session descriptor-relative binding, including root anchors, and Trusted Shell read-only direct-tool network policy with nested-interpreter rejection), with regressions for each category. macOS G2 was approved by the product owner on 2026-08-19 after the independent review package (`docs/implementation/macos-g2-review-3408413.md`); the attestation is enabled and the composition root wires the host capability. Three real DeepSeek Trusted Shell journeys, all three dogfood categories, the real-PTY terminal matrix, live DeepSeek **7/7**, live domestic MiniMax **8/8**, and `npm run acceptance:macos` **14/14** all pass at `2e8ee6b` with sanitized evidence. Remaining non-blocking items for this macOS personal work packet: the external standard-scan rerun (scanner unavailable in this session), Apple signing/notarization, and Windows 11 execution (deferred). `.omo/` remains user-owned, untracked, and preserved.

## 2026-08-19 Issue #4 `2e8ee6b` G2 approval and real-journey checkpoint

- macOS G2 approved; `MACOS_TRUSTED_SHELL_AUTO_G2_ATTESTATION.approved` is `true` (commit `926bc1a`), composition-root wiring fixed, `npm run check` stays **248/248**.
- Trusted Shell harness stale assertions fixed (`0c78f3b`, `2e8ee6b`): expect tool-event patterns now match the rendered bounded-args format, and the network-count assertion counts completed `candy_bash_network:ok` entries only.
- Real evidence at `2e8ee6b`: Trusted Shell journeys attempts 1–3 pass; dogfood `categoryScore 3/3`, `safetyFailures 0`, `offlineShellOnly=true`; terminal matrix passes; DeepSeek live gate **7/7**; MiniMax live gate **8/8**; acceptance **14/14**.
- No open P0/P1 blocker remains for the macOS personal acceptance decision; the independent scan rerun, signing/notarization, and Windows execution are recorded as separate non-blocking items.
...
- `npm run check` passes **248/248**, `npm run check:native` passes, `npm run smoke:sandbox:macos` passes the strict matrix, and `npm run acceptance:macos` passes **14/14** on macOS `26.6.1` arm64 with `realPty=true`.
