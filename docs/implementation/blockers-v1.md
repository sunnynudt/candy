...
- TUI and signed Desktop installation, packaged Node app-server lifecycle, and the remaining full ACC-12 responsiveness matrix. The unsigned Windows package and Browser action/security fixture now pass their deterministic subset; queued/active owner crash recovery passes; the latest clean-revision ten-run subset has TUI p95 `1202 ms` and Desktop p95 `1416 ms`; active tool interruption/cross-client handoff, Browser physical input-origin/adversarial coverage, concurrency, and signed installation evidence remain open.
