...
| G1-ELECTRON       | Implementation agent                     | Blocked     | Packaged Desktop acceptance                                | Secure Electron 43.2.0 window/preload/IPC and app-server client seam exists; verify independent packaged Node 22.23.2 child, fuse, restart, and graceful quit matrix on Windows 11 and macOS Sequoia.                                    |
| G1-KEYRING        | Product owner + implementation agent     | Blocked     | ACC-02 OS credential acceptance                            | Exact `@napi-rs/keyring@1.3.0` adapter and Candy-owned service/account mapping are implemented without fallback; verify signed package path against Windows Credential Manager and macOS Keychain without exposing complete credentials. |
