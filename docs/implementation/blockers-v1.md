...
| G0-MAC            | Product owner                            | In progress | Complete macOS acceptance                                  | The expanded 2026-08-11 macOS runner passes 15/15 deterministic/native/Runtime/TUI/app-server/Electron/package/recovery/packaged-handoff/Browser/responsiveness steps with 94 tests on macOS `26.5.2` arm64; packaged handoff includes stale-revision fencing and validator-descendant cleanup assertions. The latest cold-start subset is TUI p95 `958 ms` and Desktop p95 `1945 ms`. Continue with the remaining ACC-12 metrics, physical Browser input-origin/full adversarial matrix, full UI/cross-client recovery, live-provider, signing, and native-security evidence. |
