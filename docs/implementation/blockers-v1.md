...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | On clean revision `7043e286bbde645f4af7507504298234dd17011c`, the Windows 11 Pro x64 deterministic runner passes 17/17, including active validator interruption, queued/active process-restart recovery, attachment restart recovery, non-owner read-only fencing, bounded three-slot concurrency, unsigned packaged Desktop JSONL smoke, packaged active-owner/tool recovery, extended packaged Browser action/adversarial security fixture smoke, and packaged Credential Manager fixture smoke. The latest cold-start subset is TUI p95 `1169 ms` and Desktop p95 `1406 ms`; continue the dedicated checklist for packaged cross-client recovery, signed installation, complete Browser physical-input/coverage, complete G2, remaining ACC metrics, and external provider/signing gates. |
...
- TUI and signed Desktop installation, packaged Node app-server lifecycle, and the remaining full ACC-12 responsiveness matrix. The unsigned Windows package and extended Browser action/security fixture now pass their deterministic subset; queued/active owner crash recovery, attachment restart recovery, active validator interruption, non-owner cross-client fencing, bounded three-slot concurrency, and packaged active-owner/tool recovery pass; the latest clean-revision ten-run subset has TUI p95 `1169 ms` and Desktop p95 `1406 ms`; packaged cross-client recovery, Browser physical input-origin/complete adversarial coverage, renderer concurrency, and signed installation evidence remain open.
