...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | The clean-revision Windows deterministic runner passes 12/12 steps on the current Windows 11 Pro x64 host, including queued/active process-restart recovery, unsigned packaged Desktop JSONL smoke, and packaged Browser fixture smoke; continue the dedicated checklist for active tool interruption/cross-client recovery, signed installation, complete Browser, complete G2, remaining ACC metrics, and external provider/signing gates. |
...
| G3-BROWSER        | Implementation agent                     | In progress | Browser Workspace and automatic takeover claim              | Packaged Windows WebContentsView fixture now proves explicit site allow, observation, redirect/popup/permission/download default-deny, and Take Control fallback. Complete agent action bridge, screenshots, adversarial matrix, and input-origin evidence; use explicit Take Control if origin cannot be distinguished reliably. |
...
| G1-KEYRING        | Product owner + implementation agent     | In progress | ACC-02 OS credential acceptance                            | Candy-owned service/account mapping and Windows Credential Manager synthetic lifecycle (`absent -> present -> present -> absent`) pass; verify packaged path, deletion, and the existing DeepSeek account mutation only under a controlled user-authorized window, without exposing complete credentials. |
...
- TUI and signed Desktop installation, packaged Node app-server lifecycle, and the remaining full ACC-12 responsiveness matrix. The unsigned Windows package and Browser fixture now pass their JSONL/security subset; queued/active owner crash recovery passes; the latest deterministic ten-run subset has TUI p95 `1219 ms` and Desktop p95 `1565 ms`; active tool interruption/cross-client handoff, Browser actions/input-origin, concurrency, and signed installation evidence remain open.
...
- Browser Workspace agent action, screenshot attachment, packaged input-origin, Take Control, permission, redirect, download, and adversarial tests. The packaged Windows fixture now passes the default-deny and explicit Take Control subset; full Browser evidence remains open.
- Worktree and Apply Changes path, reparse-point, conflict, binary, untracked-file, and recovery matrices.
- Task/session/attachment recovery, cross-client ownership, restart, and cancellation behavior. Queued metadata and active-owner crash-interruption process-restart smoke now pass; attachment, active tool interruption, and cross-client recovery remain open.
