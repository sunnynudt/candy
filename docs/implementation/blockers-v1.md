...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | The clean-revision Windows deterministic runner passes 13/13 steps on the current Windows 11 Pro x64 host, including active validator interruption, queued/active process-restart recovery, unsigned packaged Desktop JSONL smoke, packaged Browser action/security fixture smoke, and packaged Credential Manager fixture smoke; continue the dedicated checklist for cross-client recovery, signed installation, complete Browser/adversarial coverage, complete G2, remaining ACC metrics, and external provider/signing gates. |
...
- Task/session/attachment recovery, cross-client ownership, restart, and cancellation behavior. Queued metadata, active validator interruption, and active-owner crash-interruption process-restart smoke now pass; attachment and cross-client recovery remain open.
