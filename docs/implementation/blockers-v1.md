...
| G0-MAC            | Product owner                            | In progress | Complete macOS acceptance                                  | The 2026-08-11 macOS runner passes 10/10 deterministic/native/Runtime/TUI/app-server/Electron/package steps with 94 tests on macOS `26.5.2` arm64; the sanitized report is `out/acceptance/macos/latest.md`. Continue with ACC-12 ten-run responsiveness, full recovery/Browser matrix, live-provider, signing, and native-security evidence. |
...
| G0-MAC-RUNTIME    | Product owner                            | In progress | Runtime cross-platform claim                               | The macOS `26.5.2` arm64 Runtime/session-remap smoke now passes through the Candy-owned Pi session root and Runtime recovery path; complete the cross-platform TUI/Desktop ownership and restart/session matrix before clearing this blocker. |
