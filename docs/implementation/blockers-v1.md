...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | The clean-revision Windows deterministic runner passes 11/11 steps on the current Windows 11 Pro x64 host, including queued task metadata process-restart recovery and unsigned packaged Desktop JSONL smoke; continue the dedicated checklist for active-owner/tool recovery, signed installation, Browser, complete G2, remaining ACC metrics, and external provider/signing gates. |
...
- TUI and signed Desktop installation, packaged Node app-server lifecycle, and the remaining full ACC-12 responsiveness matrix. The unsigned Windows package now passes its JSONL lifecycle smoke; the latest deterministic ten-run subset has TUI p95 `1355 ms` and Desktop p95 `1592 ms`; projection, cancellation, Browser Take Control, concurrency, and signed installation evidence remain open.
