...
| G0-MAC            | Product owner                            | In progress | Complete macOS acceptance                                  | The clean `acceptance:macos` run at source revision `e6c03a2e80385b2ae9773bdc4c0c4691b40cbb2d` passes 16/16 steps on `26.5.2` arm64. The post-push deterministic ten-run report at source revision `d9a0fe2` records TUI/Desktop cold p95 `983/2053 ms`, Runtime projection `2 ms`, validator parent-and-descendant termination `12 ms`, explicit Take Control rejection `2 ms`, and concurrent renderer frame-gap `19 ms`; all ten three-task runs have `9/9/9=>9/9/9` delivery with no event loss. The sanitized reports are `out/acceptance/macos/latest.md` and `out/acceptance/macos/responsiveness-latest.md`. User cancellation to a real Provider stream-stop request, full UI/recovery ACC-12 evidence, physical input-origin, Windows parity, signing, native-security, and live-provider evidence remain Blocked. |
...
| G2-SANDBOX        | Implementation agent + security reviewer | In progress | Shell-enabled Auto and Shell Auto Debug                    | On clean macOS `26.5.2` arm64 revision `e6c03a2e80385b2ae9773bdc4c0c4691b40cbb2d`, `npm run smoke:sandbox:macos` and the 16/16 `acceptance:macos` run pass. TypeScript `WorkspaceGuard` rejects outside/symlink reads and writes, but the raw native runner reproduces outside read/write, symlink read/write, and post-`lstat` symlink-swap escapes because the Seatbelt profile uses `(allow default)`; OS workspace containment and race resistance are therefore not proven. Native loopback network denial and detached descendant cancellation pass. No native change was made; packaging/signing and independent security review remain `Blocked`, and Shell Auto/Auto Debug stay disabled. |
| G3-BROWSER        | Implementation agent                     | In progress | Browser Workspace and automatic takeover claim              | Packaged Windows and macOS loopback fixtures prove explicit site allow, observation, structured click/type/confirmed-submit, stale/user-owner rejection, screenshot attachment ids, URL/action/selector target rejection, disallowed redirect/popup/permission/download default-deny, and the explicit Take Control fallback. macOS source revision `951283bd4c8f3c0ae8778ab50bd6ed4044f436e7` additionally proves inert prompt-injection text, a same-revision renderer navigation race with one accepted revision-fenced request, a conflicting action invalidated by Take Control, and absence of the page marker from packaged output, app-server JSONL stdout, and the Candy-owned temporary app-data tree. The last isolation evidence is deterministic and no-provider only: it does not prove live Provider propagation, complete session/protocol coverage, or physical input-origin detection. Complete hostile-page/navigation matrix and reliable input-origin evidence remain open; use explicit Take Control because automatic takeover is not claimed. |
...
| G2-NATIVE         | Security reviewer + implementation agent | In progress | Shell-enabled Auto and shell-based Auto Debug              | Windows 11 native smoke proves Job Object process ownership, complete descendant cancellation on runner close, protocol network rejection, workspace escape rejection, and junction reparse rejection. macOS G2 now has source-backed negative evidence: loopback network denial and descendant cancellation pass, while `(allow default)` permits raw outside/symlink/symlink-swap filesystem escapes. This is not OS-level workspace isolation; no Rust/Seatbelt hardening was claimed. Packaging and the independent security review remain open. |
