...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | On clean revision `8ddfbcc89f756cda1d881853202de6f74742cc42`, the Windows 11 Pro x64 deterministic runner passes 14/14, including active validator interruption, queued/active process-restart recovery, non-owner read-only fencing, unsigned packaged Desktop JSONL smoke, packaged Browser action/security fixture smoke, and packaged Credential Manager fixture smoke. The latest cold-start subset is TUI p95 `1312 ms` and Desktop p95 `1431 ms`; continue the dedicated checklist for attachment/packaged recovery, signed installation, complete Browser/adversarial coverage, complete G2, remaining ACC metrics, and external provider/signing gates. |
...
- TUI and signed Desktop installation, packaged Node app-server lifecycle, and the remaining full ACC-12 responsiveness matrix. The unsigned Windows package and Browser action/security fixture now pass their deterministic subset; queued/active owner crash recovery, active validator interruption, and non-owner cross-client fencing pass; the latest clean-revision ten-run subset has TUI p95 `1312 ms` and Desktop p95 `1431 ms`; packaged active-owner/tool interruption recovery, Browser physical input-origin/adversarial coverage, concurrency, and signed installation evidence remain open.
...
- Task/session/attachment recovery, cross-client ownership, restart, and cancellation behavior. Queued metadata, active validator interruption, active-owner crash-interruption process-restart, and non-owner read-only fencing now pass; attachment recovery and packaged cross-client handoff remain open.
