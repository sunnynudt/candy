...
| G0-MAC            | Product owner                            | In progress | Complete macOS acceptance                                  | On clean revision `a9237399ba103c77c345bcf1cfb2fc064a7fde09`, the 2026-08-11 macOS runner passes 13/13 deterministic/native/Runtime/TUI/app-server/Electron/package/recovery/responsiveness steps with 94 tests on macOS `26.5.2` arm64; TUI cold-start p95 is `1199 ms` and Desktop p95 is `2064 ms`. Continue with the remaining ACC-12 metrics, full UI/cross-client recovery and Browser matrix, live-provider, signing, and native-security evidence. |
...
| G1-ELECTRON       | Implementation agent                     | In progress | Packaged Desktop acceptance                                | macOS `26.5.2` arm64 local packaged Electron 43.2.0 + Node 22.23.2 child, JSONL round-trip, ad-hoc signature, quit cleanup, and embedded packaged Node/app-server recovery pass; Windows x64 unsigned Electron 43.2.0 + Node 22.23.2 + native runner package and JSONL smoke pass. Verify Apple/Windows signed installation, full UI recovery, and signing. |
