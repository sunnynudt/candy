...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | On clean revision `979301275a71696b1c949f27f6e00443ee9b26f3`, the Windows 11 Pro x64 deterministic runner passes 19/19, including user-cancelled long-running validator cleanup, active validator interruption, queued/active process-restart recovery, attachment restart recovery, non-owner read-only fencing, bounded three-slot concurrency, unsigned packaged Desktop JSONL smoke, packaged active-owner/tool recovery, packaged sequential cross-client handoff, extended packaged Browser action/adversarial security fixture smoke, and packaged Credential Manager fixture smoke. The latest cold-start subset is TUI p95 `1173 ms` and Desktop p95 `1396 ms`; continue the dedicated checklist for signed installation, complete Browser physical-input/coverage, complete G2, remaining ACC metrics, and external provider/signing gates. |
