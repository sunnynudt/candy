...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | The clean-revision Windows deterministic runner passes 13/13 steps on the current Windows 11 Pro x64 host, including queued/active process-restart recovery, unsigned packaged Desktop JSONL smoke, packaged Browser fixture smoke, and packaged Credential Manager fixture smoke; continue the dedicated checklist for active tool interruption/cross-client recovery, signed installation, complete Browser, complete G2, remaining ACC metrics, and external provider/signing gates. |
