...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | The clean-revision Windows deterministic runner passes 9/9 steps on the current Windows 11 Pro x64 host; continue the dedicated checklist for signed/packaged Desktop, Browser, recovery, complete G2, remaining ACC metrics, and external provider/signing gates. |
