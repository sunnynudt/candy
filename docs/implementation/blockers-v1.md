...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | The clean-revision Windows deterministic runner passes 12/12 steps on the current Windows 11 Pro x64 host, including queued task metadata process-restart recovery, unsigned packaged Desktop JSONL smoke, and packaged Browser fixture smoke; continue the dedicated checklist for active-owner/tool recovery, signed installation, complete Browser, complete G2, remaining ACC metrics, and external provider/signing gates. |
...
- TUI and signed Desktop installation, packaged Node app-server lifecycle, and the remaining full ACC-12 responsiveness matrix. The unsigned Windows package and Browser fixture now pass their JSONL/security subset; the latest deterministic ten-run subset has TUI p95 `1221 ms` and Desktop p95 `1458 ms`; projection, cancellation, Browser actions/input-origin, concurrency, and signed installation evidence remain open.
...
- Task/session/attachment recovery, cross-client ownership, restart, and cancellation behavior. A queued task metadata process-restart smoke passes; active-owner, attachment, tool interruption, and cross-client recovery remain open.
