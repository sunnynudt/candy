...
| G4-WORKTREE       | Implementation agent                     | In progress | Reviewed Apply Changes acceptance                          | Real `workspace.changes` now reports tracked/untracked/binary-safe diff data through app-server and Desktop with active-secret redaction; Desktop Apply command, restart-persisted baseline, dirty target, changed base, conflicts, and Windows path matrix remain. |
