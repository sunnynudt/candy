...
| G0-WIN            | Product owner + implementation agent     | In progress | Windows and cross-platform V1 release claims               | On clean revision `4abc88ab906edd66f0b77a707c248e2375c7640b`, the Windows 11 Pro x64 deterministic runner passes 18/18, including user-cancelled long-running validator cleanup, active validator interruption, queued/active process-restart recovery, attachment restart recovery, non-owner read-only fencing, bounded three-slot concurrency, unsigned packaged Desktop JSONL smoke, packaged active-owner/tool recovery, extended packaged Browser action/adversarial security fixture smoke, and packaged Credential Manager fixture smoke. The latest cold-start subset is TUI p95 `1180 ms` and Desktop p95 `1435 ms`; continue the dedicated checklist for packaged cross-client recovery, signed installation, complete Browser physical-input/coverage, complete G2, remaining ACC metrics, and external provider/signing gates. |
...
- TUI and signed Desktop installation, packaged Node app-server lifecycle, and the remaining full ACC-12 responsiveness matrix. The unsigned Windows package and extended Browser action/security fixture now pass their deterministic subset; queued/active owner crash recovery, attachment restart recovery, active validator interruption, user cancellation with descendant cleanup, non-owner cross-client fencing, bounded three-slot concurrency, and packaged active-owner/tool recovery pass; the latest clean-revision ten-run subset has TUI p95 `1180 ms` and Desktop p95 `1435 ms`; packaged cross-client recovery, Browser physical input-origin/complete adversarial coverage, renderer concurrency, and signed installation evidence remain open.
...
- Task/session/attachment recovery, cross-client ownership, restart, and cancellation behavior. Queued metadata, attachment restart recovery, active validator interruption, user cancellation with descendant cleanup, active-owner crash-interruption process-restart, non-owner read-only fencing, and packaged active-owner/tool interruption now pass; packaged cross-client handoff remains open.
