...
if (process.platform !== "win32" || process.arch !== "x64") {
  console.log("Packaged Windows Browser smoke skipped: not Windows x64");
  process.exit(0);
}

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const executable = path.join(root, "out", "windows", "Candy", "Candy.exe");
if (!existsSync(executable)) throw new Error("Packaged Windows Desktop executable is missing.");

const fixtureRoot = await mkdtemp(path.join(tmpdir(), "candy-browser-windows-"));
...
try {
  ...
  if (
    timedOut ||
    exit.code !== 0 ||
    !stdout.includes("packaged Windows Browser Workspace smoke ok")
  )
    throw new Error(
  ...
  console.log(
    "packaged Windows Browser Workspace smoke passed: protected local fixture with adversarial page, navigation race, and marker isolation",
  );
} finally {
