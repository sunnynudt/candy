...
if (process.platform !== "darwin" || process.arch !== "arm64") {
  console.log("Packaged macOS coding-journey smoke skipped: not macOS arm64");
  process.exit(0);
}

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const bundle = path.join(root, "out", "macos", "Candy.app", "Contents");
const executable = path.join(bundle, "MacOS", "Candy");
const packagedNode = path.join(bundle, "Resources", "node", "bin", "node");
const sandboxRunner = path.join(bundle, "Resources", "native", "candy-sandbox-runner");
if (!existsSync(executable) || !existsSync(packagedNode) || !existsSync(sandboxRunner))
  throw new Error("Packaged macOS coding-journey runtime is incomplete.");

const fixtureRoot = await mkdtemp(path.join(tmpdir(), "candy-coding-journey-macos-"));
...
const timeout = globalThis.setTimeout(() => child.kill("SIGTERM"), 90_000);
try {
  ...
  assert.equal(
    exit.code,
    0,
    `Packaged macOS coding-journey smoke exited ${exit.code ?? "null"}/${exit.signal ?? "none"}: ${output}`,
  );
  ...
  console.log(
    "Packaged macOS coding-journey smoke passed: create, stream, edit, validator, diff review, Apply, reopen transcript",
  );
} finally {
