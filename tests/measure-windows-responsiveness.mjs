...
if (!existsSync(nativeRunner))
  throw new Error("Windows Sandbox Runner is unavailable for responsiveness measurement.");
...
const fixtureRoot = await mkdtemp(path.join(tmpdir(), "candy-responsiveness-windows-"));
...
try {
  ...
  responsivenessEnvironment.USERPROFILE = home;
  responsivenessEnvironment.TEMP = temporary;
  responsivenessEnvironment.TMP = temporary;
  responsivenessEnvironment.APPDATA = appData;
  responsivenessEnvironment.LOCALAPPDATA = localAppData;
  responsivenessEnvironment.CANDY_APP_DATA_ROOT = localAppData;
...
const subsetPass =
  ...
  concurrency.pass;
const report = [
  "# Candy Windows 11 Pro x64 Responsiveness Measurement",
  "",
  ...
  "",
  "This is a ten-run Windows deterministic measurement of cold start and four local Desktop seams. It uses only Candy-owned temporary fixtures, does not access Provider or Keychain values, excludes provider and public-network latency, and does not claim complete ACC-12 or final V1 acceptance.",
  "",
...
console.log(
  `Windows responsiveness subset ${subsetPass ? "passed" : "failed"}: TUI p95=${tui.p95} ms, Desktop p95=${desktop.p95} ms, projection p95=${runtimeProjection.p95} ms, cancellation p95=${cancellationProcessTree.p95} ms, Browser p95=${browserTakeControl.p95} ms, concurrency frame-gap p95=${concurrency.frameGapP95} ms; report=out/acceptance/windows/responsiveness-latest.md`,
);
...
function runDesktopResponsiveness(command, args, cwd, env) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = globalThis.setTimeout(() => {
      child.kill();
      globalThis.setTimeout(() => child.kill(), 2_000).unref();
    }, 90_000);
